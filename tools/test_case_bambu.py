...
def main() -> None:
    ...
    if filament_ids != [1, 2] or plate.get("filament_change_times") != 1:
        raise AssertionError(
    ...
    layer_two = gcode.index("; Z_HEIGHT: 0.4")
    layer_totals = {
        int(match.group(1))
    ...
    if any(match.start() > layer_two for match in re.finditer(r"^M620 S1A", gcode, re.MULTILINE)):
        raise AssertionError("Bambu requests black filament above the first layer")
    ...
    if not prime_tower_z or max(prime_tower_z) > 0.2 + 1e-6:
        raise AssertionError(
